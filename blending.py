...
def reduce_layer(image, kernel=generatingKernel(0.4)):
    ...
    """
    typed_image = image.astype(np.float64)
    typed_kernel = kernel.astype(np.float64)
    padded_image = cv2.copyMakeBorder(typed_image,2,2,2,2,cv2.BORDER_REFLECT_101)
    convolved_image = scipy.signal.convolve2d(padded_image, typed_kernel, mode='valid')
    out_image = convolved_image[::2,::2] # credit:  https://stackoverflow.com/q/10198747
    return out_image
...
def expand_layer(image, kernel=generatingKernel(0.4)):
    ...
    """
    r, c = image.shape
    upsampled_image = np.zeros((2*r, 2*c)) # credit:  https://stackoverflow.com/q/27125959
    typed_image = image.astype(np.float64)
    typed_kernel = kernel.astype(np.float64)
    upsampled_image[::2,::2] = typed_image
    padded_image = cv2.copyMakeBorder(upsampled_image, 2, 2, 2, 2, cv2.BORDER_REFLECT_101)
    convolved_image = scipy.signal.convolve2d(padded_image, typed_kernel, mode='valid')
    convolved_image = convolved_image * 4
...
def gaussPyramid(image, levels):
    ...
    for x in range(levels+1):
        if x==0:
            gauss_list.append(typed_image)
        else:
            image_k = reduce_layer(gauss_list[x-1])
            gauss_list.append(image_k)
    return gauss_list



def imageCrop(expansion, smaller):
    current_shape = list(expansion.shape)
    smaller_shape = list(smaller.shape)
    excess = np.array(current_shape) - np.array(smaller_shape)
    excess_rows = excess[0]
    excess_columns = excess[1]
    if excess_rows>0 and excess_columns>0:
        result = np.copy(expansion[:-excess_rows,:-excess_columns])
        return result
    if excess_rows>0 and excess_columns==0:
        result = np.copy(expansion[:-excess_rows,:])
        return result
    if excess_rows==0 and excess_columns>0:
        result = np.copy(expansion[:,:-excess_columns])
        return result
    if excess_rows==0 and excess_columns==0:
        result = np.copy(expansion)
        return result
...
def laplPyramid(gaussPyr):
    ...
    lapl_list = []
    for counter, image in enumerate(gaussPyr):
        if counter < (len(gaussPyr) - 1):
            expansion = expand_layer(gaussPyr[counter + 1])
            cropped_image = imageCrop(expansion, image)
            laplacian_image = image - cropped_image
            lapl_list.append(laplacian_image)
        else:
            lapl_list.append(image)
    return lapl_list
...
def blend(laplPyrWhite, laplPyrBlack, gaussPyrMask):
    ...
    output_list = []
    for counter, image in enumerate(laplPyrWhite):
        result = ( image * gaussPyrMask[counter] ) + ( laplPyrBlack[counter] * (1-gaussPyrMask[counter]) )
...
def collapse(pyramid):
    ...
    level = len(pyramid)-1
    for counter in range(level,-1,-1):
        expansion = expand_layer(pyramid[counter])
        next_layer = pyramid[counter-1]
        cropped_image = imageCrop(expansion, next_layer)
        result = cropped_image + next_layer
        pyramid[counter-1]=result
    return pyramid[0]
